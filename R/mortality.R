...
#' show.year.mortality(mortality.with.stats, seq(2019, 2020))
  ...
  my.plot <- dat %>%
    ...
    #
    geom_crossbar(aes(x = day, y = meanPre2020, ymin=meanPre2020 - sdPre2020, ymax=meanPre2020 + sdPre2020, fill = 'Error bars from 2009-19'), alpha = 0, size = .1, width = 1) +
    geom_crossbar(aes(x = day, y = meanPre2020, ymin=meanPre2020 - sdPre2020, ymax=meanPre2020 + sdPre2020, color = 'Error bars from 2009-19', fill = 'Error bars from 2009-19'), alpha = .4, size = .1, width = 1, show.legend = FALSE) +
    #
    ...
    geom_blank(data = data.frame(x = rep(5, 5), y = 0.5, 
                                 group = c("Min/Max from 2009-19", "Error bars from 2009-19", "Smooth regression from 2009-19", "Year", "Smooth regression from Year")), 
    ...
    scale_color_manual("", values = c("darkgray", "darkgreen", "green", "red", 'darkred'), breaks=c("Min/Max from 2009-19", "Error bars from 2009-19", "Smooth regression from 2009-19", "Year", "Smooth regression from Year")) +
    scale_fill_manual("Legend_no", values = c("darkgray", "darkgreen", "green", "red", 'darkred'), guide=FALSE, breaks=c("Min/Max from 2009-19", "Error bars from 2009-19", "Smooth regression from 2009-19", "Year", "Smooth regression from Year")) +
