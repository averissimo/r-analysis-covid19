...
```{r}
suppressWarnings({
  p <- ommit.start(after.100.dat$all, 'death', params$confirmed.start, filter.states = countries) %>% ggplotly() %>% 
    ggplotly()
})
...
```{r}
suppressWarnings({
  p <- ommit.start(after.100.dat$all, 'death', params$confirmed.start, filter.states = countries, log2.flag = TRUE) %>% ggplotly() %>% 
    ggplotly()
})
...
```{r}
suppressWarnings({
  p <- ommit.start(after.100.dat$all, 'death', params$confirmed.start, filter.states = countries, per.100k.flag = TRUE) %>% ggplotly() %>% 
    ggplotly()
})
