...
## Visualizations in last `r params$last.days` days {.tabset .tabset-fade .tabset-pills}
...
## Confirmed cases / Deaths of previous 4 days *(how it tracks over time)* {.tabset .tabset-fade .tabset-pills}
...
plot.death.vs.cases <- function(country.filter, always.include = c('Portugal', 'Germany', 'Iran', 'Spain', 'Italy', 'Korea, South')) {
...
### Data for the countries in *Confirmed cases vs. Deaths*
...
```{r, echo=FALSE}
hospital.beds.original <- wb(indicator = "SH.MED.BEDS.ZS") %>%
    mutate(value = value * 100,
   indicator = "Hospital beds (per 100,000 people)")

hospital.beds.norm <- hospital.beds.original
for (ix.name in names(convert.names)) {
    hospital.beds.norm <- hospital.beds.norm %>%
        arrange(country) %>%
        ungroup %>%
        mutate(country = replace(country, country == ix.name, convert.names[[ix.name]]))    
}

hospital.beds <- hospital.beds.norm %>% 
    group_by(country) %>%
    arrange(desc(date)) %>%
    summarise(date = first(date),
              value = first(value))

beds.vs.cases <- left_join(death.vs.cases, hospital.beds, by = 'country') %>%
    mutate(ratio.confirmed = cases.confirmed / value) %>%
    mutate(ratio.death = cases.death / value) 

# death.vs.cases$country[!death.vs.cases$country %in% hospital.beds$country]
# hospital.beds.norm$country %>% unique %>% sort
```


```{r, echo=FALSE}
plot.beds.vs.cases <- function(country.filter, always.include = c('Portugal', 'Iran', 'Germany', 'Spain', 'Italy', 'Korea, South')) {
    my.tbl <- beds.vs.cases %>%
        filter(country %in% (c(always.include, country.filter) %>% unique))
    return(
        my.tbl %>%
            ggplot(aes(x = cases.confirmed, y = value, color = country)) +
                    ...
                    labs(x = 'Confirmed Cases per 100k population', 
                         y = 'Hospital beds per 100k population', 
                         title = 'Hospital beds vs. Cases per 100k population', 
                         subtitle = 'percentage shows the hospital beds per confirmed cases and deaths, respectively',
                         caption = last.date.string) +
                    theme_minimal() +
                    theme(legend.position = 'none')
        )
...
## Nurses and Midwifes *(per 100k population)*


```{r, echo=FALSE}
nurses.original <- wb(indicator = "SH.MED.NUMW.P3") %>%
    mutate(value = value * 100,
   indicator = "Nurses and midwives (per 100,000 people)")

nurses.norm <- nurses.original
for (ix.name in names(convert.names)) {
    nurses.norm <- nurses.norm %>%
        arrange(country) %>%
        ungroup %>%
        mutate(country = replace(country, country == ix.name, convert.names[[ix.name]]))    
}
...
nurses.vs.cases <- left_join(death.vs.cases, nurses, by = 'country') %>%
    mutate(ratio.confirmed = cases.confirmed / value) %>%
    mutate(ratio.death = cases.death / value) 
...
```{r, echo=FALSE}
physicians.original <- wb(indicator = "SH.MED.PHYS.ZS") %>%
    mutate(value = value * 100,
   indicator = "Physicians (per 100,000 people)")

physicians.norm <- physicians.original
for (ix.name in names(convert.names)) {
    physicians.norm <- physicians.norm %>%
        arrange(country) %>%
        ungroup %>%
        mutate(country = replace(country, country == ix.name, convert.names[[ix.name]]))    
}

physicians <- physicians.norm %>% 
    group_by(country) %>%
    arrange(desc(date)) %>%
    summarise(date = first(date),
              value = first(value))

    physicians.vs.cases <- left_join(death.vs.cases, physicians, by = 'country') %>%
    mutate(ratio.confirmed = cases.confirmed / value) %>%
    mutate(ratio.death = cases.death / value) 
