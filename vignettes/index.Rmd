...
it.dat.raw <- download.it.data()
pt.dat.raw <- download.pt.data() 

source.by       <- '{world.dat$source}, {it.dat.raw$source} and {pt.dat.raw$source}' %>% glue
last.date.world <- format(max(world.dat$data$date), '%Y/%m/%d')
last.date.it    <- format(max(it.dat.raw$data$date), '%Y/%m/%d')
last.date.pt    <- format(max(pt.dat.raw$data$date), '%Y/%m/%d')

last.date        <- '{last.date.world} (EU CDC), {last.date.it} (Protezione Civile) and {last.date.pt} (PT DGS)' %>% glue
last.date.string <- paste0('Latest date from ', last.date)

it.dat <- it.dat.raw$data %>%
    ungroup() %>%
    mutate(state = 'Italy') %>%
    group_by(state, state.code, date, type) %>%
...
my.corona.original <- world.dat$data %>% 
    ...
    ungroup() %>%
    bind_rows(pt.dat.raw$data) %>% 
    add_row(state = it.dat$state, type = it.dat$type, date = it.dat$date, cases = it.dat$cases, cumul = it.dat$cumul, population = it.population, state.code = it.dat$state.code) %>%
...
convert.names <- eu.convert.names.arr
```
...
countries          <- c('Italy', 'Spain','Germany', 'Portugal', 'USA', 'France', 'Switzerland') %>% sort
...
countries.always.include <- c('Portugal', 'USA', 'Germany', 'Spain', 'Italy', 'Korea, South')
