...
age.group %>% 
  ...
  scale_fill_gradient(low = "orange", high = "red", na.value = '#00000020') +
  scale_x_discrete(position = "top") +
  labs(title = 'Mortality by age group from \'March 15 to Now\'', subtitle = 'All years show the same period', caption = "Data retrieved on {format(Sys.Date(), '%B,%d %Y')}" %>% glue::glue()) + 
  xlab('Age group') +
  ylab('Year') +
  theme_minimal() +
  theme(legend.position = 'none')
