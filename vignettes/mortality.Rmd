...
```{r, echo=TRUE}
pois.levels <- c('2020-lambda.5', 
                 'pre2020-lambda', 
                 'pre2020-lambda.5', 
                 'pre2020-ic95.5', 
                 'pre2020-ic99.5')
pois.labels <- c('2020 (fit)',  # this is new
                 #
                 '2020',
...
mortality.plot.poisson <- mortality.pois.transf %>% 
  ...
  geom_smooth(aes(y = value, color = '2020 (fit)'), alpha = .4, data = mortality.pois.transf %>% filter(group == '2020'), span = 0.1, method = 'loess', formula = y ~ x, na.rm = TRUE, show.legend = FALSE, se = FALSE) +
  ...
  labs(title = '2020 vs Baseline using Poisson distribution', 
