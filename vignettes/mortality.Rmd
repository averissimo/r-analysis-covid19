...
Updated at `r format(Sys.time(), '%A, %d %B %Y (%d %Y-%m-%d %H:%m %Z)')`
...
external.causes %>% 
  ...
  labs(title = 'External causes of death from \'March 15 to Now\'', subtitle = 'All years show the same period', caption = 'Accidents, suicides and others. It excludes natural causes or diseases') + 
...
mortality.pois.transf <- mortality.pois  %>% 
  ...
  mutate(lambda.5 = zoo::rollapply(lambda, width = 7, FUN = function(x) {mean(x,na.rm=TRUE)}, partial = TRUE, fill = NA, align = 'left'),
  ...
  filter(new.group %in% c('2020-lambda.5', 'pre2020-lambda', 'pre2020-lambda.5', 'pre2020-ic95.5', 'pre2020-ic99.5')) %>% 
  mutate(ymin = min(value)) %>% 
  arrange(day, group)
  
mortality.pois.transf %>% 
  ...
  scale_color_discrete("", labels = c('2020','Base line for 2015-19','Confidence interval 95% 2015-19','Confidence interval 99% 2015-19')) +
  labs(title = '2020 vs Baseline using Poisson distribution', subtitle = 'Data calculated with centered average of 7 days') +
  theme_minimal() +
...
age.group %>% 
  ...
  scale_fill_gradient(low = "orange", high = "red", na.value = '#00000020') +
  scale_x_discrete(position = "top") +
  labs(title = 'Mortality by age group from \'March 15 to Now\'', subtitle = 'All years show the same period') + 
  xlab('Age group') +
  ylab('Year') +
  theme_minimal() +
  theme(legend.position = 'none')
...
age.group %>% 
  ...
  scale_fill_gradient(low = "orange", high = "red", na.value = '#00000020') +
  scale_x_discrete(position = "top") +
  labs(title = 'Mortality by age group', subtitle = 'All years show the same period (last year may be incomplete)') + 
  xlab('Age group') +
  ylab('Year') +
  theme_minimal() +
  theme(legend.position = 'none')
