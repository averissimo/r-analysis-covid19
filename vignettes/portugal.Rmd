...
for (age.ix in seq_along(ixs)) {
  ...
  print(age.data.all.new %>%
    ...
    scale_y_discrete(limits = age.data$type %>% unique %>% sort) + 
    scale_fill_viridis_d('', end = .8) + 
    labs(title = 'New confirmed cases from {format(age.data$date %>% max, "%B %d")}' %>% glue,
         subtitle = 'In parenthesis the predicted deaths based on current mortality rate (total predicted deaths: {sum(age.data.all.new$predicted.death) %>% round(digits = 2)})' %>% glue,
         caption = 'Mortality rate = \'deaths\' / \'confirmed cases\'',
         y = 'Age group',
         x = 'Confirmed Cases') +
    theme_minimal() + 
  ...
  print(age.data.all.new %>%
    ...
    scale_x_continuous('', labels = function(ix) { return(abs(ix)) }) +
    scale_fill_viridis_d('', end = .8) + 
    labs(title = 'New deaths from {format(age.data$date %>% max, "%B %d")}' %>% glue,
         subtitle = '',
         caption = '',
         y = 'Age group',
         x = 'Deaths') +
